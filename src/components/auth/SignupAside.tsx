...
function Bullet({ children }: { children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-3">
      <span
        className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-rd-terra-100"
        aria-hidden="true"
      >
        <Check className="h-3 w-3 text-rd-terra-700" />
      </span>
      <span className="text-sm leading-relaxed text-rd-ink-700">{children}</span>
    </li>
  );
}
...
function Reassure({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <li className="flex items-center gap-2 text-xs font-medium text-rd-ink-700">
      <span className="text-rd-navy-700" aria-hidden="true">{icon}</span>
      {children}
    </li>
  );
}
...
export function SignupAside() {
  ...
  return (
    <div className="max-w-lg">
      ...
      <p className="mt-4 text-base leading-relaxed text-rd-ink-700">
        {t(
