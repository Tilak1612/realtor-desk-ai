...
const NotesTab = ({ contactId }: NotesTabProps) => {
  ...
  const fetchNotes = useCallback(async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from("contact_notes").select("*").eq("contact_id", contactId).order("is_pinned", { ascending: false }).order("created_at", { ascending: false });
            ...
            notes.map((note) => (
              <div key={note.id} className="p-4 rounded-lg border bg-card space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {note.is_pinned && <Badge variant="secondary" className="flex-shrink-0"><Pin className="h-3 w-3 mr-1" />Pinned</Badge>}
                </div>
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <div className="flex gap-2">
                    {editingNoteId === note.id ? (
                      ...
                      <>
                        <Button size="sm" variant="ghost" onClick={() => { setEditingNoteId(note.id); setEditContent(note.content); }}><Edit className="h-3 w-3" /></Button>
                        <Button size="sm" variant="ghost" onClick={() => handleTogglePin(note.id, !!note.is_pinned)}><Pin className="h-3 w-3" /></Button>
                        <Button size="sm" variant="ghost" onClick={() => handleDeleteNote(note.id)}><Trash2 className="h-3 w-3" /></Button>
                      </>
                    )}
                  </div>
                </div>
