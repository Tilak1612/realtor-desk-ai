    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        {similarContacts.map((similar) => (
          <div
            key={similar.id}
            onClick={() => navigate(`/contacts/${similar.id}`)}
          ...
          >
            <Avatar className="h-10 w-10">
              <AvatarFallback className="bg-primary text-primary-foreground">
                {similar.first_name?.[0]?.toUpperCase() || "?"}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm truncate">
                {similar.first_name} {similar.last_name}
              </p>
              <p className="text-xs text-muted-foreground truncate">
                {similar.source || "No source"}
              </p>
            </div>
            <Badge variant={getScoreBadgeColor(similar.ai_score)}>
              {similar.ai_score || 0}
            </Badge>
          </div>
        ))}
